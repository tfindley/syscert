...
func accountKey(base, directoryURL string) (signer crypto.Signer, existed bool, err error) {
	...
	if data, err := os.ReadFile(path); err == nil { // #nosec G304 -- account-key path under the syscert-owned store, not user input
		...
		if block == nil {
			return nil, false, fmt.Errorf("account key %s is not PEM", path)
		}
		key, err := x509.ParseECPrivateKey(block.Bytes)
		if err != nil {
			return nil, false, fmt.Errorf("parse account key %s: %w", path, err)
		}
		return key, true, nil // loaded from disk → the account already exists
	}
	...
	if err := os.WriteFile(path, pemBytes, 0o600); err != nil {
		return nil, false, fmt.Errorf("write account key %s: %w", path, err)
	}
	return key, false, nil // freshly created
}
