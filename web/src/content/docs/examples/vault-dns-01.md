...
With dns-01 the validation runs the other way. Vault's own resolver queries
`_acme-challenge.<fqdn>`, so the TXT record your DNS provider publishes has to be visible to
Vault. Watch split-horizon and internal DNS here.

The role-scoped directory (`.../roles/web/acme/directory`) pins issuance to the `web` role; drop
back to `.../pki/acme/directory` for the mount-wide form.

For EAB, mint a credential with `vault write -f pki/roles/web/acme/new-eab`. The returned `id` is
your `kid`, and `key` is `SYSCERT_EAB_HMAC`, copied verbatim. Skip `[acme.eab]` entirely if the
mount's `eab_policy` doesn't ask for it. The full how-to is [EAB → Vault](/docs/eab/vault/). One
version caveat: your Vault build needs a PKI ACME that exposes `dns-01`.
