...
lede: One container, two processes — possible, but it has a real downside. Read the trade-off and the mitigation before you use it.
---

> **Read the trade-off below before using this pattern.** If a single-container constraint is not
> hard, use the [sidecar pattern](/docs/containerisation/sidecar/) instead.

The embedded pattern runs `syscert --interval` and the application (e.g. nginx) in a **single
container**. It is documented here because it is feasible and sometimes required (constrained PaaS,
image-size budgets, demos), but it carries a real trade-off that requires an explicit mitigation.

## The trade-off

Two responsibilities in one container means an ambiguous failure mode:

- If **syscert** fails (e.g. a persistent ACME error), nginx keeps running — serving the existing
  (possibly expiring) certificate **silently**. Nothing tells you the renewal broke.
- If **nginx** crashes, syscert keeps renewing — but nothing is serving the certificate.

Neither failure causes the container to exit, so Docker's restart policy never fires. Both
failures are silent unless you actively monitor the log output.

## Required mitigation: restart on either exit

The `entrypoint.sh` in the example starts both processes and **exits the script when either process
exits**. Docker's `restart: unless-stopped` then restarts the whole container, which at minimum
...
For a more robust setup, replace this script with a real process supervisor:
...
A process supervisor allows fine-grained per-process restart policies (restart syscert on failure
without restarting nginx, and vice versa) and proper PID 1 signal handling.
...
Use `challenge = "dns-01"` in `syscert.toml` — no inbound ports, no conflict with nginx's
...
Pass DNS-provider credentials at runtime via `--env-file` or Docker secrets. Never bake them into
the image.
...
The store at `/var/lib/syscert` must be owned by the uid that syscert runs as. In the Dockerfile
above, `adduser` creates uid `1001`; `chown syscert:nginx` sets ownership so nginx (gid `101`) can
read distributed files.

If using a named volume (recommended for persistence), fix its owner:
...
- You are operating at any scale beyond a single instance.
